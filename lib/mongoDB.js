...
export class mongoDB {
  constructor(url, dbName = "emilia", options = defaultOptions) {
    this.url = url;
    this.dbName = dbName;
    this.options = { ...options, dbName: this.dbName }; // Menambahkan dbName ke options
  ...
  async read() {
    try {
      ...
      if (!this._data) {
        const [_ignored, _data] = await Promise.all([
...
export class mongoDBV2 {
  ...
  async write(data) {
    try {
      if (!this.lists || !data) throw new Error("Invalid data atau lists");
      const listDoc = [];
