...
class Uploader {
  ...
  async top4top(buffer) {
    ...
    try {
      ...
      const informationSection = cheerio.load(output)(
        "div.alert-warning ul.index_info li span",
      );

      spinner.succeed(chalk.green("Uploaded to Top4Top"));

      const result = {
        imageThumbnailLink: informationSection
          .find(".thumb_div_tag a")
          .attr("href"),
        directImageUrl: informationSection
          .find('.inputbody p.btitle:contains("رابط الصورة المباشر") + input')
          .val(),
        deleteLink: informationSection
          .find('.inputbody p.btitle:contains("رابط الحذف") + input')
          .val(),
      };
  ...
  async uploadToDiscdn(content) {
    ...
    try {
      ...
      const res = await fetch(
        "https://discord.com/api/v9/channels/1180731738176094228/messages",
        {
          method: "POST",
          headers: {
            Authorization:
              "Bot MTE4MDcyODk4MjAzNjA0MTczOA.GtqzcS.grSeXjgylvsY_e7YxYi4acHKIrYTabaOnubOx8",
          },
          body: formData,
        },
      );
