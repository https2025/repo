...
let handler = async (
  ...
  if (room_all)
    return m.reply(
      `Kamu sedang bermain sambung kata di chat lain, selesaikan game kamu terlebih dahulu`,
    );
  if (id in conn.skata) {
    ...
    if (room.status == "play") {
      if (!room.waktu._destroyed && !room.diam)
        return conn
          .reply(
            m.chat,
            `Hii @${m.sender.split`@`[0]}, Masih ada game berlangsung di chat ini\nTunggu hingga game berakhir\nLalu ikut bergabung`,
            room.chat,
    ...
    if (text == "start" && room.status == "wait") {
      ...
      if (member.includes(m.sender)) return m.reply(`Kamu sudah ikut di list`);
