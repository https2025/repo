...
Ketik *'kiri'* untuk bergerak ke kiri.`;
  ...
  conn.koboy[m.chat] = {
    ...
    key,
    oldkey: key,
    earnedExp: [100, 200, 300, 400, 900, 100, 1000, 100, 2000, 40000, 7000],
    earnedMoney: [10000, 20000, 30000, 40000, 90000, 10000, 10000, 100, 200000, 4000000, 700000],
    sender: m.sender,
    moveCount: 0,
    maxMoves: 5,
    roomId: m.chat,
...
handler.before = async (m, { conn }) => {
  ...
  if (m.quoted || m.quoted.id == key) {
    ...
    user.exp = (user.exp || 0) + earnedExps;
      delete conn.koboy[m.chat];
      return conn.reply(m.chat, `🎉 Selamat! @${sender.split('@')[0]} berhasil mengejar penjahat! 🎉\n\n💰 Mendapatkan money senilai *${earnedMoneys}*\n🔼 Dan *${earnedExps}* EXP\n`, m, { mentions: [sender] });
    } else if (moveCount === maxMoves) {
    conn.sendMessage(m.chat, { delete: oldkey });
      delete conn.koboy[m.chat];
      return conn.reply(m.chat, `😔 Kamu kalah! @${sender.split('@')[0]}.`, m, { mentions: [sender] });
    }
