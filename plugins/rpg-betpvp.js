...
let handler = async (m, { conn, args, usedPrefix, command }) => {
  ...
  if (
    ...
    return m.reply("Selesaikan judi mu yang sebelumnya");
  if (
    Object.values(conn.judipvp).find(
      (room) =>
    ...
    return m.reply(`Orang yang kamu tantang sedang bermain judipvp bersama orang lain :(`);
  ...
  conn.judipvp[id] = {
    chat: await conn.reply(
      m.chat,
      `@${m.sender.split("@")[0]} Mengajak @${musuh.split("@")[0]} Berjudi Apakah Kamu Mau Menerimanya? (Y/N)`,
      m,
      {
