...
let handler = async (m, { conn, command, args }) => {
  ...
  if (m.quoted && m.quoted.mtype) {
    ...
    forwardedNewsletterMessageInfo: {
      ...
      newsletterName: '📡ping : 0.0011  ||  ⌜ Emilia-MD (エミリア) ⌟ © cifumo'
    }
  }
      doc.video = {
        url: link,
      ...
      doc.caption = teks;
      doc.mentions = Object.keys(global.db.data.users).filter(
          (key) => global.db.data.users[key].registered,
        )
      doc.contextInfo = {
    forwardingScore: 9999,
    isForwarded: true,
    ...
    await conn
      .sendMessage("status@broadcast", doc, {
        ...
        statusJidList: Object.keys(global.db.data.users).filter(
          (key) => global.db.data.users[key].registered,
        ),
      })
      .then((res) => {
