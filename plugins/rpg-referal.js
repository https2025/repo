...
let handler = async (m, { conn, usedPrefix, text }) => {
  ...
  if (text) {
    if ("ref_count" in users[m.sender])
      return m.reply("Tidak bisa menggunakan kode referal!");
    ...
    users[link_creator].legendary += xp_link_creator + extra;
    users[m.sender].legendary += xp_first_time;
    users[m.sender].ref_count = 0;
    m.reply(
      `
Selamat!
+${xp_first_time} Legendary
`.trim(),
    );
    m.reply(
      `
Seseorang telah menggunakan kode referal kamu
+${xp_link_creator + extra} Legendary 
`.trim(),
      link_creator,
    );
  } else {
    ...
    let share_text = `
Dapatkan ${xp_first_time} Legendary untuk yang menggunakan link/kode referal dibawah ini
...
`.trim();
    m.reply(
      `
Dapatkan ${xp_link_creator} Legendary untuk setiap pengguna baru yang menggunakan kode referal kamu
...
${Object.entries(xp_bonus)
  .map(([count, xp]) => `${count} Orang = Bonus ${xp} Legendary`)
  .join("\n")}
`.trim(),
    );
  }
