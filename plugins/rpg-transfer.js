...
`.trim();
  ...
  if (user[type] * 1 < count)
    return m.reply(
      `Your *${global.rpg.emoticon(type)}${type}${special(type)}* is less *${count - user[type]}*`,
    );
  if (/money/i.test(type) && _user[type] * 1 > 99999998)
    return m.reply(
      `${type}${global.rpg.emoticon(type)} @${who.split("@")[0]} sudah limit`,
      false,
      { mentions: [who] },
    );
  ...
  if (previous > user[type] * 1 && _previous < _user[type] * 1)
    ...
    m.reply(
      `*––––––『 TRANSFER 』––––––*\n*📊 Status:* Failted\n*📍 Item:* ${count} ${global.rpg.emoticon(type)}${type}${special(type)}\n*📨 To:* @${(who || "").replace(/@s\.whatsapp\.net/g, "")}`,
      null,
      { mentions: [who] },
    );
  }
