...
let handler = async (m, { conn, text, command, usedPrefix }) => {
  ...
  if (!cmd)
    return m.reply(
      `Masukan nama command! \n\nContoh : \n${usedPrefix + command} diamond`,
    );
  if (typeof store[cmd] !== "undefined")
    return m.reply("Command tersebut telah ada!");
  ...
  if (
    ...
    !caption &&
    !m.quoted &&
    !/image\/(jpe?g|png)|video\/mp4/.test(mime)
  )
    return m.reply(
      `Masukan isi command \n\nContoh : \n${usedPrefix + command} diamond@List Harga Diamond...`,
    );
  if (!cmd && !/image\/(jpe?g|png)|video\/mp4/.test(mime))
    return m.reply(`Hanya support image atau video`);
