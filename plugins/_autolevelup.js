...
export async function before(m) {
  ...
  if (chat.autolevelup || user[m.sender].autolevelup) {
    if (canLevelUp(user[m.sender].level, user[m.sender].exp, 38)) {
      ...
      while (canLevelUp(user[m.sender].level, user[m.sender].exp, 38))
        user[m.sender].level++;
...
`.trim();
        ...
        const pp = await this.profilePictureUrl(m.sender, "image").catch((_) =>
          fs.readFileSync("./src/avatar_contact.png"),
        );
        const name = user[m.sender].registered
          ? user[m.sender].name
          : this.getName(m.sender);
        try {
          let img = await canvafyRank(
            pp,
            name,
            "online",
            user[m.sender].level,
            member.indexOf(m.sender),
            user[m.sender].exp - min,
            xp,
          );
